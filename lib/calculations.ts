import { ComparisonInput, ComparisonOutput, Vehicle, ICEVehicle, EVVehicle, VehicleResultSummary, YearlyBreakdownComponent } from './types';
...
interface LoanResult { monthlyPayment: number; totalPaidWithinHorizon: number; yearlyPayments: number[]; }
function loanPayments(v: Vehicle, horizonYears: number): LoanResult {
  ...
  if (!termYears) {
    return { monthlyPayment: 0, totalPaidWithinHorizon: 0, yearlyPayments: Array.from({ length: horizonYears }, () => 0) };
  }
...
function computeICE(v: ICEVehicle, annualKm: number, horizon: number): { yearly: YearlyBreakdownComponent[]; totalFuel: number; totalMaint: number; resaleOffset: number; totalLoan: number; totalCost: number; costPerKm: number | null; } {
  const annualFuelLiters = annualKm && v.fuelEfficiencyKmPerLiter > 0 ? annualKm / v.fuelEfficiencyKmPerLiter : 0;
...
function computeEV(v: EVVehicle, annualKm: number, horizon: number): { yearly: YearlyBreakdownComponent[]; totalFuel: number; totalMaint: number; resaleOffset: number; totalLoan: number; totalCost: number; costPerKm: number | null; } {
  ...
  for (let y = 1; y <= horizon; y++) {
    ...
    yearly.push({ year: y, fuelOrElectricity: elecY, maintenance: maintY, loan: loanY, depreciation: depreciationY, cumulativeTotal: cumulative });
  }
